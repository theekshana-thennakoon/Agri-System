...
<?php
    $sqlr = "SELECT DISTINCT(farmer) as farmer FROM chats where officer = '$user' group by farmer";
    $resultr = $conn->query($sqlr);
    if ($resultr->num_rows > 0){
        $row = "";
        while($rowr = $resultr->fetch_assoc()){
            $farmer = $rowr['farmer'];
            $sql = "SELECT * FROM users where email = '{$farmer}'";
            ...
            if ($result->num_rows > 0){
                ...
                if ($row == ""){
                    $row = "<tr><td><a href = 'officer-chat.php?farmer={$farmer}'>{$fname} {$lname}</a></td></tr>";
                }
                else{
                    $row = "<tr><td><a href = 'officer-chat.php?farmer={$farmer}'>{$fname} {$lname}</a></td></tr>";
                }
