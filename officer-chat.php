...
<?php
    if (isset($_GET['farmer'])){
        $_SESSION['farmer'] = $_GET['farmer'];
        $officer = $_SESSION['farmer'];
    }
...
?>
    ...
    <form action = "x.php" method = 'post'>
        <?php
            $sqlr = "SELECT * FROM chats where officer = '$user' and farmer = '$farmer'";
            $resultr = $conn->query($sqlr);
            if ($resultr->num_rows > 0){
                $row = "";
                while($rowr = $resultr->fetch_assoc()){
                    if ($rowr['fmsg'] !=""){
                        echo "<br><div style = 'float:left;font-size:18px;color:#fff;padding:2%;width:40%;border:none;background:#5CC074;border-bottom-left-radius:10px;border-bottom-right-radius:10px;border-top-right-radius:10px;'>" . $rowr['fmsg'] . "</div><br><br><br>";
                    }
                    if ($rowr['omsg'] !=""){
                        echo "<br><div style = 'float:right;font-size:18px;color:#fff;padding:2%;width:40%;border:none;background:#0f0;border-bottom-left-radius:10px;border-bottom-right-radius:10px;border-top-left-radius:10px;'>" . $rowr['omsg'] . "</div><br><br><br>";
                    }
                }
            }
        ?>
        ...
        <input type = 'submit' name = 'smsgofficer' Value = 'Send Message' style = 'padding:1%;width:20%;'>
    </form>
    </div>
</body>
</html> 